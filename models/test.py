...
import copy
def test_mnist7(net_g, dataset_test, test_other_idx, test_7_idx, test_bar7_idx, args):
    data_loader_other = DataLoader(dataset_test,
                        batch_size=args.bs,
                        sampler=torch.utils.data.sampler.SubsetRandomSampler(test_other_idx))
    data_loader_7 = DataLoader(dataset_test,
                        batch_size=args.bs,
                        sampler=torch.utils.data.sampler.SubsetRandomSampler(test_7_idx))
    data_loader_bar7 = DataLoader(dataset_test,
                        batch_size=args.bs,
                        sampler=torch.utils.data.sampler.SubsetRandomSampler(test_bar7_idx))
    _, acc_other = test_acc(copy.deepcopy(net_g), data_loader_other, len(test_other_idx), args)
    _, acc_7 = test_acc(copy.deepcopy(net_g), data_loader_7, len(test_7_idx), args)
    _, acc_bar7 = test_acc(copy.deepcopy(net_g), data_loader_bar7, len(test_bar7_idx), args)
